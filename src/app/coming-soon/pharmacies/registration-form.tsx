...
const requiredFields: FieldName[] = ["pharmacyName", "managerName", "city", "phone", "email"];
...
const validate = (values: FormValues) => {
  ...
  if (!values.pharmacyName.trim()) errors.pharmacyName = "نام داروخانه ضروری است.";
...
export function PharmacyRegistrationForm() {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    addRegistration({
      ...
      extra: values.license.trim() ? { license: values.license.trim() } : undefined,
      createdAt: new Date().toISOString(),
      status: "pending",
      createdBy: "user",
    });
    router.push("/coming-soon/pharmacies/registered");
  };
      ...
      <div className="grid gap-x-3 gap-y-2 md:grid-cols-2">
        <div className={fieldWrapper}>
          <input
            ...
            onChange={(event) => updateValue("pharmacyName", event.target.value)}
            onBlur={() => setTouched((prev) => ({ ...prev, pharmacyName: true }))}
            aria-invalid={Boolean(showError("pharmacyName"))}
            aria-describedby="pharmacyName-error"
            className={`${inputBase} ${showError("pharmacyName") ? errorClasses : normalClasses}`}
          />
          <span
            id="pharmacyName-error"
            className="mt-0.5 block min-h-[14px] text-xs text-rose-200/80"
            aria-live="polite"
          >
            {showError("pharmacyName") ? errors.pharmacyName : ""}
          </span>
        </div>
        ...
        <div className={fieldWrapper}>
          <input
            type="text"
            placeholder="کد مجوز (اختیاری)"
            value={values.license}
            onChange={(event) => updateValue("license", event.target.value)}
...
export function PharmacyRegistrationCard() {
